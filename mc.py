...
def seed_tournament(states: dict[str, TeamState], rng: np.random.Generator, w_pred: float = W_PRED, w_res: float = W_RES):
    """
    Returns:
    ...
    MODIFIED: Uses random tiebreaker for AQ ties instead of deterministic selection_score/adjem
    """
    ...
    for conf, lst in conf_to_teams.items():
        ...
        # Sort by conference wins and win percentage
        def sort_key(s: TeamState):
            g = s.conf_w + s.conf_l
        ...
        # Find all teams tied for first place
        if not lst_sorted:
            continue
        ...
        # Collect all teams with the same conf_w and win percentage
        tied_teams = [best]
        for team in lst_sorted[1:]:
            if sort_key(team) == best_key:
                tied_teams.append(team)
            else:
                break
        
        # If there's a tie, select randomly; otherwise take the single leader
        if len(tied_teams) > 1:
            top = rng.choice(tied_teams)
        else:
            top = best
...
def run_one_sim(
    ...
    for _, g in schedule.iterrows():
        ...
        if team_win == 1:
            ...
            states[team].q_l[q] += 1
            states[opp].q_w[q] += 1
    # Pass rng to seed_tournament for random tiebreaker
    field_df, aq_list, selection68 = seed_tournament(states, rng, w_pred, w_res)
